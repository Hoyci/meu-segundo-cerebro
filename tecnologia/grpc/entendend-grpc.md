...
Além disso, o **HTTP/2** introduz o conceito de **Multiplexação**, que permite enviar e receber múltiplas requisições simultaneamente através de uma única conexão **TCP**, reduzindo drasticamente a latência. Diferentemente do HTTP/1.1, que não possui multiplexação, cada arquivo ou recurso solicitado exige que o navegador abra múltiplas conexões TCP, sejam elas sequenciais ou paralelas. Por fim, o protocolo é **bidirecional**: ao contrário do **HTTP/1.1**, onde apenas o cliente inicia a comunicação, o **HTTP/2** permite que o servidor envie dados de forma proativa enquanto a conexão estiver aberta, funcionando de forma semelhante ao websockets.
